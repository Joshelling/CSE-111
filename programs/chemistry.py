...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for i in range(0, len(symbol_quantity_list)):
        (ele, ele_qty) = symbol_quantity_list[i]
        for key, value in periodic_table.items():

            ele_key = ele[periodic_table]
        ...
        # element_match = periodic_table[element_key]
        # element_mass = periodic_table[ATOMIC_MASS_INDEX]
...
def main():
    ...
    new_quant_list = parse_formula(formula, new_table)
    print(compute_molar_mass(new_quant_list, new_table))

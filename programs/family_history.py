...
def count_genders(people_dict):
    ...
    for item in people_dict.items():
      key = item[0]
      person_gender = item[1]
      gender = person_gender[GENDER_INDEX]
      if gender == "M":
        total_male += 1 
      elif gender == "F":
        total_female += 1
    print(f'The total amount male {total_male}, the total amount of females is {total_female}.')
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for key, person_key in marriages_dict.items():
        
           

            husband_key = person_key[HUSBAND_KEY_INDEX]
            wife_key = person_key[WIFE_KEY_INDEX]
            marriage_year = person_key[WEDDING_YEAR_INDEX]
            ...
            print(f'''{husband_name} was married to {wife_name} in {marriage_year} at
            the age of {husband_age} and his wife was {wife_age} years old.''')
